...
int convert_digit(int value)                       // fonction de conversion en hexadecimal
{
	...
	if(value<=9) valTens = value +0x30;          // pour les unit�s
...
void Aff_valeur(int c)
{
	...
	digit2=(c & ( 0x0F00))>>0x08;
	digit1=(c & ( 0xF000))>>0x0C;
	v[3]=(digit4);			           // placement dans un tableau de 4 caract�res + conversion
	v[2]=(digit3);
	v[1]=(digit2);
	v[0]=(digit1);
...
/**
 * Afficher un valeur en decimal
 */
void Aff_Dec_Val (int c) {
	Aff_valeur(convert_Hex_Dec(c));
}
